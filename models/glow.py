...
"""Glow/normalizing-flow building blocks used by MAGANet decoders."""


def compute_same_pad(kernel_size, stride):
    """Compute padding values that preserve spatial resolution for a conv op."""
...
def gaussian_likelihood(mean, logs, x):
    """Return summed Gaussian log-likelihood over spatial and channel dims."""
    p = gaussian_p(mean, logs, x)
    return torch.sum(p, dim=[1, 2, 3])


def gaussian_sample(mean, logs, temperature=1):
    """Sample from diagonal Gaussian with optional temperature scaling."""
...
def unsqueeze2d(input, factor):
    """Inverse transform of `squeeze2d` (channel to spatial)."""
    if factor == 1:
        return input
...
class ActNorm2d(_ActNorm):
    """2D activation normalization wrapper."""
...
class LinearZeros(nn.Module):
    """Linear layer initialized to zero with trainable output scale."""
...
class Conv2d(nn.Module):
    """Convolution layer with optional ActNorm and custom init std."""
...
class Conv2dZeros(nn.Module):
    """Zero-initialized convolution with trainable log-scale."""
...
class Permute2d(nn.Module):
    """Channel permutation layer (reverse or random-shuffle)."""
...
class Split2d(nn.Module):
    """Split-prior layer used in multi-scale Glow architecture."""
...
class SqueezeLayer(nn.Module):
    """Squeeze/unsqueeze wrapper compatible with flow API."""
...
class InvertibleConv1x1(nn.Module):
    """Learned invertible 1x1 convolution with optional LU parameterization."""
...
def get_block(in_channels, out_channels, hidden_channels):
    """Build coupling-network block used inside a flow step."""
    block = nn.Sequential(
...
class FlowStep(nn.Module):
    """Single reversible flow step: ActNorm -> Permute -> Coupling."""
...
class FlowNet(nn.Module):
    """Multi-level normalizing flow network with squeeze/split structure."""
...
class Glow(nn.Module):
    """Full Glow model with optional class-conditioning heads."""
...
class ReallyInvertibleFlowNet(nn.Module):
    """Non-splitting flow variant used by MAGANet decoder action space."""
